...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        lowest = [sys.maxsize - 1, legal[0]]

        topBelief = [max(dist, key = lambda x : dist[x]) for dist in livingGhostPositionDistributions ]
        for x in legal:
            temp = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,x), ghost) for ghost in topBelief)
            if lowest[0] > temp:
                lowest[0] = temp
                lowest[1] = x
        return lowest[1]
